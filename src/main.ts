...
program
  ...
  .option(
    ...
    "12-work seed phrase for wallet bot",
  )
  .option(
    "-t --auth-token <auth_token>",
    "anypay api auth token for wallet bot",
  )
  .option("-a --api-base <api_base>", "anypay walletbot backend api base url")
  .option(
    "-p --prometheus-enabled <prometheus_enabled>",
    "enable prometheus metrics",
  )
  .option(
    "-h --http-api-enabled <http_api_enabled>",
    "server json data over http api to clients",
  )
  .option(
    "-w --websocket-url <websocket_url>",
    "url for websockets connection to server",
  )
  .option(
    "-e --websocket-enabled <websocket_enabled>",
    "true or false, connect to websocket server",
  )
...
program.command("start").action(async () => {
  ...
  if (!seed_phrase) {
    console.error(
      "WALLETBOT_SEED_PHRASE environment variable or --seed-phrase command line option is required",
    )
    process.exit(1)
  }
  ...
  if (!auth_token) {
    console.error(
      "WALLETBOT_AUTH_TOKEN environment variable or --auth-token command line option is required",
    )
    process.exit(1)
  }
  ...
  const websocket_url =
    options.websocketUrl || config.get("WALLETBOT_WEBSOCKET_URL")
  ...
  const newOptions = {
    ...
    http_api_enabled:
      http_api_enabled || default_walletbot_options.http_api_enabled,
    websocket_enabled:
      websocket_enabled != undefined
        ? websocket_enabled
        : config.get("WALLETBOT_WEBSOCKET_ENABLED"),
  ...
  const wallet_bot_options: WalletBotOptions = {
    ...
    ...(newOptions.http_api_enabled !== undefined && {
      http_api_enabled: newOptions.http_api_enabled,
    }),
    ...(newOptions.websocket_enabled !== undefined && {
      websocket_enabled: newOptions.websocket_enabled,
    }),
    ...(newOptions.websocket_url !== undefined && {
      websocket_url: newOptions.websocket_url,
    }),
  }
...
program.command("seed-phrase").action(() => {
...
program
  ...
  .requiredOption(
    "-v --value <value>",
    "value amount to pay in the quote currency (default to satoshis)",
  )
  .requiredOption(
    "-q --quote <currency>",
    "quote currency for determining the amount",
  )
  ...
  .option(
    ...
    "url of server to send payment notification",
  )
...
program
  ...
  .requiredOption(
    "-u --uid <invoice_uid>",
    "uid of anypay wallet bot invoice to cancel",
  )
  .action(async (options) => {
    ...
    const app = anypay.app({ apiKey: auth_token })

    console.log('cancel-payment.options', options)

    const { data } = await axios.delete(`https://anypayx.com/r/${options.uid}`, {
      auth: {
        username: auth_token,
        password: "",
      }
    ...
    console.log('cancel-payment.result', data)

    // TODO: Implement cancel payment request using anypay.cancelInvoice
  })

program.command("list-balances").action(async () => {
  const options = program.opts()
